...
@section('content')
    ...
    @if(isset($data['banners']) && $data['banners']->count() > 0)
        @include('web.templates.saeta.parts.banners', ['items' => $data['banners']])
    @endif

    @if(isset($data['fast_access']) && $data['fast_access']->count() > 0)
        @include('web.templates.saeta.parts.fast_access', ['items' => $data['fast_access']])
    @endif

    @if(isset($data['tablon']) && $data['tablon']->count() > 0)
        @include('web.templates.saeta.parts.tablon', ['avisos' => $data['tablon']])
    @endif
